...
const ReportDetails = () => {
	...
	const [nonVCchildren, setNonVCChildren] = useState<any>(report?.report.children);

	const sendReportToSubamdin = () => {
		// reports?.map((item: any) => return setChildren([...children, item._id]));
		console.log("Children reports: ", nonVCchildren);
		axios
			.post("/subadmin/sendreport", nonVCchildren)
			.then((res) => {
				alert.show("Report successfully sent to admin!", {
					type: "success",
				});
			})
			.catch((err) => {
